...
int
filter_distance (IplImage * img, int distance)
{
...
void
freenect::set_filter (int from, int to)
{
    filter_from = from;
    filter_to = to;
}
...
void
freenect::depth_cb (freenect_device * dev, void *v_depth, uint32_t timestamp)
{
    int i;
...
void
freenect::rgb_cb (freenect_device * dev, void *rgb, uint32_t timestamp)
{
    pthread_mutex_lock (&gl_backbuf_mutex);
...
void *
freenect::freenect_threadfunc (void *arg)
{
    int accelCount = 0;
...
void
freenect::init (short user_device_number)
{
    if (error != 0)
      {
    ...
    int res =
        pthread_create (&freenect_thread, NULL, freenect::freenect_threadfunc,
                        NULL);
    if (res)
      {
